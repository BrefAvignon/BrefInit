...
#			fileEncoding="Latin1"		# original tables seem to be encoded in Latin1 (ANSI)
	...
	{	# load individual table
		...
		lastname.conv <- load.senate.conversion.file(FILE_SENAT_CONV_NOMSFAM)
		idx <- match(indiv.last.names, lastname.conv[, COL_CORREC_VALAVT])
		indiv.last.names <- lastname.conv[idx, COL_CORREC_VALAPR]
		
		# normalize first names
		indiv.first.names <- trimws(normalize.proper.nouns(remove.diacritics(indiv.table[,COL_SENAT_ELU_PRENOM])))
		firstname.conv <- load.senate.conversion.file(FILE_SENAT_CONV_PRENOMS)
		idx <- as.integer(firstname.conv[, COL_CORREC_ROW])
		indiv.first.names <- firstname.conv[idx, COL_CORREC_VALAPR]
		...
		dpts.conv <- load.senate.conversion.file(FILE_SENAT_CONV_DPTS)
		idx <- match(indiv.dpt.names, dpts.conv[, COL_CORREC_VALAVT])
		indiv.dpt.names <- dpts.conv[idx, COL_CORREC_VALAPR]
...
#			fileEncoding="Latin1"		# original tables seem to be encoded in Latin1 (ANSI)
		...
		nuances.conv <- load.senate.conversion.file(FILE_SENAT_CONV_NUANCES)
		idx <- match(indiv.pol.nuances, nuances.conv[, COL_CORREC_VALAVT])
		indiv.pol.nuances <- nuances.conv[idx, COL_CORREC_VALAPR]
		...
		occup.conv <- load.senate.conversion.file(FILE_SENAT_CONV_PRO)
		idx <- match(indiv.occ.names, occup.conv[, COL_CORREC_VALAVT])
		indiv.occ.names <- occup.conv[idx, COL_CORREC_VALAPR]
...
#############################################################################################
senate.integrate.data <- function(data, type, compare=FALSE)
